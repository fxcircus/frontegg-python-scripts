...
def get_users_with_pagination():
    ...
    while next_link:
        log_to_execution_file("Getting next page! " + next_link)

        # Extract the _offset value from next_link using regular expression
        offset_match = re.search(r'_offset=(\d+)', next_link)
        if offset_match:
            ...
            items_arr.extend(page_items_arr)
            next_link = next_page_res.get("_links", {}).get("next", "")
        else:
...
fieldnames = [
    'id', 'name', 'email', 'verified', 'createdAt', 'isLocked',
    'provider', 'phoneNumber', 'tenantId', 'tenantIds', 'mfaEnrolled'
]
...
with open('users.csv', 'w', newline='', encoding='utf-8') as csvfile:
    ...
    for user in users_result['items']:
        # Ensure that all fields are present and convert lists to strings if necessary
        user_row = {}
